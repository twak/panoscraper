...
GSVPANO.PanoDepthLoader = function (parameters) {
    ...
    this.load = function(panoId) {
        ...
        $.ajax({
            ...
            .done(function(data, textStatus, xhr) {
                var decoded, results;
                 ...
                 try {
                    ...
                    results = self.parse(decoded);
                 } catch(e) {
                     console.error("Error loading depth map for pano " + panoId + "\n" + e.message + "\nAt " + e.filename + "(" + e.lineNumber + ")");
                     results = self.createEmptyDepthMap();
                 }
                if(self.onDepthLoad) {
            ...
            .fail(function(xhr, textStatus, errorThrown) {
                ...
                var results = self.createEmptyDepthMap();
                if(self.onDepthLoad) {
                    self.results = results;
                    self.onDepthLoad();
                }
    ...
    this.parsePlanes = function(header, depthMap) {
        ...
        for(i=0; i<header.numberOfPlanes; ++i) {
            ...
            if (byteOffset + 15 > depthMap.buffer.byteLength )
                break;
    ...
    this.computePlaneMap = function(header, indices, planes) {
        
        var w = header.width, h = header.height, x, y;
        var planeMap = new Uint8Array(w*h);
        var planeIdx;

        for(y=0; y<h; ++y) {
            for(x=0; x<w; ++x) {
                planeMap[y*w + (w-x-1)] =  indices[y*w + x];
            }
        }

        var tidyPlanes = [];
        for (var i = 0; i < planes.length; i++) {
            tidyPlanes.push( planes[i].n.concat([planes[i].d]) );
        }

        return { planeMap: planeMap, tidy: tidyPlanes, w : w, h: h};
    }
    ...
    this.parse = function(depthMap) {
        ...
        planeMap = self.computePlaneMap(header, data.indices, data.planes);

        return {
            depth: depthMap.depthMap, 
            plane: planeMap.planeMap, 
            tidy: planeMap.tidy,
            w: depthMap.width, h : depthMap.height };
    }
